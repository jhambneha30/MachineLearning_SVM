...
def remove_outliers_util(temp_x, label):
    ...
    for x in temp_x:
        ...
        # print("dist: ", dist)
        if label == 0 and 0.85 <= dist <= 1.1:
            final_list.append(x)
        elif label == 1 and 0.3 <= dist <= 0.59:
            final_list.append(x)
...
X_arr, y_arr = load_h5py('/home/nehaj/Desktop/Assignment2_ML/data/data_4.h5')
...
plt.xticks(())
plt.yticks(())
plt.savefig('plots/data4_kernel_noOutlier')
plt.show()
